...
namespace arquitectSoft.View.Wpf.Panels
{
    ...
    public partial class PuertasPanel : UserControl
    {
        ...
        // La columna del botón de duplicar se añade a mano una sola vez: las autogeneradas se
        // rehacen cada vez que cambia el ItemsSource, pero las manuales se conservan.
        private void DgNuevas_AutoGeneratedColumns(object sender, EventArgs e)
        {
            foreach (var c in DgNuevas.Columns)
                if (c is DataGridTemplateColumn) return;   // ya está puesta

            DgNuevas.Columns.Add(new DataGridTemplateColumn
            {
                Header = "",
                Width = 40,
                CanUserSort = false,
                IsReadOnly = true,
                CellTemplate = (DataTemplate)FindResource("TplDuplicar")
            });
            DgNuevas.Columns[DgNuevas.Columns.Count - 1].DisplayIndex = 0;
        }

        /// <summary>
        /// Duplica la puerta de esa fila —mismo código, apertura, acabado Y MEDIDAS— y deja
        /// la copia justo debajo. Es la forma de meter varias puertas iguales sin teclear la
        /// misma altura y anchura una y otra vez: se rellena una, y se duplica.
        /// La copia estrena nomenclatura automática (la nomenclatura no se puede repetir).
        /// </summary>
        private void Duplicar_Click(object sender, RoutedEventArgs e)
        {
            var origen = sender as FrameworkElement;
            var drv = origen != null ? origen.DataContext as DataRowView : null;
            if (drv == null || drv.Row.RowState == DataRowState.Deleted) return;
        ...
        private void Exportar_Click(object sender, RoutedEventArgs e)
        {
            ...
            string folder;
            folder = SelectorCarpeta.Elegir(Owner, "Carpeta donde guardar el Excel", null);
